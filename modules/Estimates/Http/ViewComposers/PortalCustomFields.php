...
class PortalCustomFields
{
    ...
    public function compose(View $view)
    {
        ...
        $custom_fields = Field::where('location', $custom_field_location)->enabled()->orderBy('name')->get();

        if (false !== strpos($view->getName(), '.edit') || false !== strpos($view->getName(), '.show')) {
            $item = $view->getData()[$custom_field_model];

            $custom_field_values = FieldValue::record($item)->get();
        }
        ...
        if ($custom_fields) {
            foreach ($custom_fields as $custom_field) {
                ...
                $view_path = 'custom-fields::field';
                if (false !== strpos($view->getName(), '.show')) {
                    $view_path = 'custom-fields::field_show';
                }

                // Push to a stack
                $view->getFactory()->startPush(
                    $custom_field->fieldLocation->sort_order,
                    view($view_path, compact($compact))
                );
            }
        }
    }
