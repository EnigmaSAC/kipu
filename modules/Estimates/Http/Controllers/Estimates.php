...
class Estimates extends Controller
{
    ...
    {
        $builder = Document::estimate()->with('contact', 'transactions');
        $estimates = $builder->collect(['document_number' => 'desc']);
        $summaryItems   = $this->getTotalsforEstimates($builder);

        return view('estimates::estimates.index', compact('estimates', 'summaryItems'));
    }

    private function getTotalsforEstimates(Builder $builder): array
    {
        $totals = [
            'expired' => 0,
            'invoiced' => 0,
        ...
        $builder->future()->each(function ($document) use (&$totals) {
            ...
            // Check if the document is open or expired
            {
                if ('expired' === $document->status) {
                    $totals['expired'] += $document->getAmountConvertedToDefault();
                }

                    elseif ('invoiced' === $document->status) {
                    $totals['invoiced'] += $document->getAmountConvertedToDefault();
                    }

                    else {
                    $totals['open'] += $document->getAmountConvertedToDefault();
                }
            }
        });
    ...
    public function convertToInvoice(Document $estimate)
    {
        ...
        if (0 === count($estimate->items)) {
            flash(trans('estimates::general.messages.item_required'))->error();
